...
“metro+”: “METRO +”, “ميترو”: “METRO +”,
“تيجان”: “TIJAN”, “وسط”: “MIDST”,
}

# ✅ FIX 1: Map sheet tab names to correct project names
...
# ✅ FIX 1: Use correct project name from TAB_TO_PROJECT map
project_name = TAB_TO_PROJECT.get(ws_title, ws_title)
return {
“sheet”: project_name,
...
if s in cnt: cnt[s] += 1
if sum(cnt.values()) > 0:
# ✅ FIX 1: Use correct project name as key
pname = TAB_TO_PROJECT.get(ws.title, ws.title)
res[pname] = cnt
return res
...
if ml.strip() in ["تمام","ok","okay","تم","موافق","next"]:
    current_unit.pop(uid, None)
    # ✅ FIX 2: Clear budget results when done
    if uid in user_context:
        user_context[uid].pop("last_budget_results", None)
...
if any(k in ml for k in ["مقدم","ميزانية","إجمالي","اجمالي","مليون","ألف","الف"]) and any(c.isdigit() for c in msg):
    try:
        ...
        if tmin > 0 and tmax > 0:
            ...
            add_h(uid,"user",msg)
            # ✅ FIX 2: Save budget results for project filtering
            if uid not in user_context:
                user_context[uid] = {}
            user_context[uid]["last_budget_results"] = exact or close
            if exact:
                out = f"✅ *{len(exact)} وحدة في النطاق:*\n\n" + "\n\n---\n\n".join([fmt_unit(u) for u in exact[:15]])
                out += "\n\n💡 _قولي اسم المشروع لو عايز تفلتر_"
...
# ✅ FIX 2: Filter budget results by project name
if user_context.get(uid, {}).get("last_budget_results"):
    dp2 = detect_project(msg)
    if dp2:
        all_results = user_context[uid]["last_budget_results"]
        filtered = [u for u in all_results if dp2.lower() in u["sheet"].lower()]
        if filtered:
            user_context[uid].pop("last_budget_results", None)
            out = f"✅ *{len(filtered)} وحدة في {dp2}:*\n\n" + "\n\n---\n\n".join([fmt_unit(u) for u in filtered[:15]])
            await update.message.reply_text(out, parse_mode="Markdown")
            return
        else:
            await update.message.reply_text(f"مفيش وحدات في {dp2} بالميزانية دي.")
            return
